...
void stringcopy(char *c1, char *c2)//user defined string copy
{
    int i = 0;
    while (c2[i] != '\0')
    {
...
{
    free_node *nptr = (free_node *)(malloc(sizeof(free_node)));//creating a free node using malloc
    nptr->size = size;
    nptr->start = start;
    nptr->end = size + start - 1;
    nptr->next = NULL;
    nptr->prev = NULL;
    return nptr;//returning freenode after initializing
}
